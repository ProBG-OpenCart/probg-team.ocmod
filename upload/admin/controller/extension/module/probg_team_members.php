<?php
class ControllerExtensionModuleProbgTeamMembers extends Controller {
    private $error = array();

    public function index() {
        $data = $this->load->language('extension/module/probg_team');
        $this->load->language('extension/module/probg_team_members');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_setting_module->addModule('probg_team_members', $this->request->post);
            } else {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/probg_team_members', 'user_token=' . $this->session->data['user_token'] . (isset($this->request->get['module_id']) ? '&module_id=' . (int)$this->request->get['module_id'] : ''), true)
        );

        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('extension/module/probg_team_members', 'user_token=' . $this->session->data['user_token'], true);
        } else {
            $data['action'] = $this->url->link('extension/module/probg_team_members', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . (int)$this->request->get['module_id'], true);
        }
        ...
        $this->response->setOutput($this->load->view('extension/module/probg_team_members', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/probg_team_members')) {
